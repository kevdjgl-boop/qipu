 <!-- Scripts de Firebase -->
    <script type="module">
        ...
        async function initializeFirebase() {
            if (IS_FIREBASE_MODE) {
                ...
                try {
                    ...
                    // 1. Autenticar: Prioridad al token de Canvas o al inicio anónimo
                    if (initialAuthToken) {
                        await signInWithCustomToken(auth, initialAuthToken);
                    } else {
                        // En servidores públicos (GitHub Pages), se usará esta ruta.
                    ...
                    onAuthStateChanged(auth, (user) => {
                        if (user) {
                            userId = user.uid;
                            const statusText = IS_CANVAS_ENV ? 'Canvas' : 'Servidor (GitHub)';
                            document.getElementById('auth-status').innerHTML = `
                                ✅ Autenticado en ${statusText}. Tu ID es: <strong>${userId}</strong>.
                                Cambios en tiempo real (Firebase).
                            `;
                            // Mostrar el ID del usuario actual en el Sidebar
                            document.getElementById('current-user-id').textContent = `ID de Usuario Actual: ${userId}`;
                    ...
                    document.getElementById('auth-status').textContent = `❌ Error al conectar con Firebase: ${error.message}. Forzando modo local.`;
                ...
                `;
                // Ocultar la sección de sincronización si no estamos en modo Firebase
        ...
        async function saveState(updates) {
            ...
            if (IS_FIREBASE_MODE && db && userId !== 'loading') {
        ...
        function setupParticipantModalListeners() {
            ...
            // --- Lógica de Sincronización (Solo en Modo Firebase) ---
            if (IS_FIREBASE_MODE) {
                document.getElementById('sync-form').addEventListener('submit', async (e) => {
                    e.preventDefault();
                    const syncUserId = document.getElementById('sync-user-id').value.trim();
                    if (!syncUserId || syncUserId === userId) {
                        showModal("ID inválido o es tu ID actual.");
                        return;
                    }
                    
                    try {
                        // 1. Cerrar la sesión actual (anónima)
                        await auth.signOut();
                        ...
                        // Si la sincronización es exitosa, onAuthStateChanged manejará la actualización.
                        showModal("Sincronización exitosa. Datos cargados del nuevo dispositivo.");
                        toggleSidebar(false);

                    } catch (error) {
                        console.error("Error al sincronizar dispositivo:", error);
                        showModal(`Error de sincronización. Asegúrate de que el ID sea correcto. (Código: ${error.code})`);
                        // Volver a iniciar sesión anónimamente si falla
                        await signInAnonymously(auth);
                    }
                });
            }
