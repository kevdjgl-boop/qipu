...
function calculateSummary(state, filteredExpenses) {
  ...
  filteredExpenses.forEach((expense) => {
    ...
    // Si es tarjeta de crédito con dueño, el dueño es quien paga (a menos que el pagador sea un invitado)
    ...
    expensesInCycle.forEach((exp) => {
      ...
      if (exp.type === "shared") {
        ...
        if (exp.items && exp.items.length > 0 && numPayees > 0) {
          ...
          appState.participants.forEach(p => { participantShares[p.id] = 0; });
          let guestShares = {};
          expGuests.forEach((gName, gIdx) => { guestShares[`guest_${gIdx}`] = 0; });
          ...
          exp.items.forEach(item => {
            ...
            Object.keys(assignments).forEach(pId => {
              ...
              if (assignedQty > 0) {
                ...
                if (participantShares[pId] !== undefined) {
                  participantShares[pId] += cost;
                } else if (pId.startsWith("guest_") || pId.startsWith("guest-")) {
                  guestShares[pId] = (guestShares[pId] || 0) + cost;
                }
              }
            });
            ...
            if (assignedUnitsSum === 0 && item.assignedTo) {
              ...
              if (participantShares[item.assignedTo] !== undefined) {
                participantShares[item.assignedTo] += itemTotal;
              } else if (item.assignedTo.startsWith("guest_") || item.assignedTo.startsWith("guest-")) {
                guestShares[item.assignedTo] = (guestShares[item.assignedTo] || 0) + itemTotal;
              }
            }
            ...
            if (unassignedUnits > 0) {
              ...
              expGuests.forEach((gName, gIdx) => {
                const gKey = `guest_${gIdx}`;
                guestShares[gKey] = (guestShares[gKey] || 0) + sharePerPerson;
              });
            }
          });
          ...
          // Deudas de participantes registrados hacia el pagador
          appState.participants.forEach(p => {
            const totalPersonShare = (participantShares[p.id] || 0) + remainingSharePerPerson;
          ...
          // Deudas de invitados hacia el pagador
          expGuests.forEach((gName, gIdx) => {
            const gKey = `guest_${gIdx}`;
            const totalGuestShare = (guestShares[gKey] || 0) + remainingSharePerPerson;
            if (gKey !== currentFinancier && totalGuestShare > 0.01) {
              const guestLabel = `${gName} (Invitado)`;
              group.debtors[guestLabel] = (group.debtors[guestLabel] || 0) + totalGuestShare;
              group.totalReimburse += totalGuestShare;
            }
          });
          ...
          expGuests.forEach((gName, gIdx) => {
            const gKey = `guest_${gIdx}`;
            if (gKey !== currentFinancier) {
              const guestLabel = `${gName} (Invitado)`;
              group.debtors[guestLabel] = (group.debtors[guestLabel] || 0) + split;
              group.totalReimburse += split;
            }
          });
        }
